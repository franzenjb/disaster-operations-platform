...
export function ICS215GuidedEntryV3() {
  ...
  const addResourceNeed = () => {
    if (currentResource.category && currentResource.type) {
      setResourceNeeds([...resourceNeeds, currentResource as ResourceNeed]);
      setCurrentResource({
        category: undefined,
        type: '',
        quantity: 1,
                    ...
                    {resourceNeeds.map((res, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                          ...
                          <span className="ml-2 text-sm">× {res.quantity}</span>
                        </div>
                        <div className={`px-2 py-1 rounded text-xs font-medium ${
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCurrentResource({...currentResource, quantity: parseInt(e.target.value) || 1})}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        min="1"
                      />
                    </div>
